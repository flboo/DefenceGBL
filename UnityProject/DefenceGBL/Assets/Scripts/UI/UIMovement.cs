...
namespace VRStandardAssets.Utils
{
    ...
    public class UIMovement : MonoBehaviour
    {
		...
		[SerializeField] private bool m_RotateByUpDown;
        ...
        private void Update()
        {
            ...
            if (m_RotateWithCamera)
            {
				...
				if(m_RotateByUpDown)
				{
					targetDirection = Vector3.ProjectOnPlane (m_Camera.forward, Vector3.zero).normalized; //Vector3.up
				} 
				else
				{
					targetDirection = Vector3.ProjectOnPlane (m_Camera.forward, Vector3.up).normalized;
				}
